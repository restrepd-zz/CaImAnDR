function drgCaImAnBatchPerSessionEventsPerTriallickvsdFFOptFlow(choiceBatchPathName,choiceFileName)

%This code does a movement and lick derivative analysis per trial
...
for no_trial_windows=1:total_trial_windows
    ...
    if sum(dFF_trial_mask)>0
        ...
        for winNo=1:szwins(1)
            ...
            handles_outs.pcorwwin(no_trial_windows).Dim_out(winNo) = nansum(eig(cov(Signal)))^2/nansum(eig(cov(Signal)).^2);
            
            
            switch no_trial_windows
                case 1
                    fprintf(1, ['Dimensionality for window %d and percent correct <65  = %d\n'], winNo, handles_outs.pcorwwin(no_trial_windows).Dim_out(winNo));
                case 2
...
save([caimanhandles.caimandr_choices.outPathName caimanhandles.caimandr_choices.outFileName(1:end-4) '_slopestest.mat'],'handles_outs')
