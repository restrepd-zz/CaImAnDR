...
%This is used to generate Ma et al 2020 Fig. 2
...
title('Normalized dFF odor window')


%Perform the glm for LDA percent correct 
fprintf(1, ['\n\nglm for normalized dFF for S+ and S- odor window\n'])
...
% Do ranksum/t test
fprintf(1, ['\n\nRanksum or t-test p values for normalized dFF odor window\n'])
try
...
for ii=1:6
    ...
    these_exists=ones(1,length(handles.caimandr_choices.outFileName));
    these_exists(1,:)=dFFsp_existw1(:,ii);
    meandFFspw1(1,ii)=mean(norm_dFFspw1(logical(these_exists),ii));
    these_dFF=ones(1,sum(these_exists));
    these_dFF(1,:)=norm_dFFspw1(logical(these_exists),ii);
    CIdFFspw1(:,ii) = bootci(1000, {@mean, these_dFF})';
    ...
    these_exists=ones(1,length(handles.caimandr_choices.outFileName));
    these_exists(1,:)=dFFsm_existw1(:,ii);
    meandFFsmw1(1,ii)=mean(norm_dFFsmw1(logical(these_exists),ii));
    these_dFF=ones(1,sum(these_exists));
    these_dFF(1,:)=norm_dFFsmw1(logical(these_exists),ii);
    CIdFFsmw1(:,ii) = bootci(1000, {@mean, these_dFF})';
...
%mmPVG04 is GCaMP6f
plot(perCorr,norm_dFFspw1(1,:),'-o','MarkerSize',10,'Color',[1 0.7 0.7])
plot(perCorr,norm_dFFsmw1(1,:),'-o','MarkerSize',10,'Color',[0.7 0.7 1])

%mmPVG02 is GCaMP6s
plot(perCorr,norm_dFFspw1(2,:),'-s','MarkerSize',10,'Color',[1 0.7 0.7])
plot(perCorr,norm_dFFsmw1(2,:),'-s','MarkerSize',10,'Color',[0.7 0.7 1])

%mmG7f09 is GCaMP7f
plot(perCorr,norm_dFFspw1(3,:),'-x','MarkerSize',10,'Color',[1 0.7 0.7])
plot(perCorr,norm_dFFsmw1(3,:),'-x','MarkerSize',10,'Color',[0.7 0.7 1])

%mmPVG05 is GCaMP6f
plot(perCorr,norm_dFFspw1(4,:),'-o','MarkerSize',10,'Color',[1 0.7 0.7])
plot(perCorr,norm_dFFsmw1(4,:),'-o','MarkerSize',10,'Color',[0.7 0.7 1])


plot(perCorr,meandFFspw1,'-or','LineWidth',2)
plot(perCorr,meandFFsmw1,'-ob','LineWidth',2)

for ii=1:6
     plot([perCorr(ii) perCorr(ii)],CIdFFspw1(:,ii),'-r','LineWidth',2,'MarkerSize',10)
     plot([perCorr(ii) perCorr(ii)],CIdFFsmw1(:,ii),'-b','LineWidth',2,'MarkerSize',10)
end
...
xlabel('Percent correct')
ylabel('Normalized dFF')
title('Normalized dFF pre odor')


%Perform the glm for LDA percent correct 
fprintf(1, ['\n\nglm for normalized dFF for S+ and S- pre odor\n'])
tbl = table(glm_norm_dFF.data',glm_norm_dFF.perCorr',glm_norm_dFF.spm',...
    'VariableNames',{'dFF','perCorr','spm'});
mdl = fitglm(tbl,'dFF~perCorr+spm+perCorr*spm'...
    ,'CategoricalVars',[3])
...
% Do ranksum/t test
fprintf(1, ['\n\nRanksum or t-test p values for normalized dFF pre odor\n'])
try
    [output_data] = drgMutiRanksumorTtest(norm_dFF_stats);
    fprintf(1, '\n\n')
catch
