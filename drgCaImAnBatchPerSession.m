function drgCaImAnBatchPerSession
...
for filNum=1:caimanhandles.caimandr_choices.no_files
    ...
    %Read the file
    cd(caimanhandles.caimandr_choices.PathName)
    load(caimanhandles.caimandr_choices.FileName{filNum})
...
for ii=1:num_odor_trials-sliding_window+1
    ...
    perCorr(ii+(sliding_window/2))=100*(no_Hits+no_CRs)/sliding_window;
    
    if ii==1
        perCorr(ii:(sliding_window/2))=perCorr(ii+(sliding_window/2));
    end
    if ii==num_odor_trials-sliding_window+1
        perCorr(ii+(sliding_window/2)+1:num_odor_trials)=perCorr(ii+(sliding_window/2));
    end
end
...
for filNum=2:caimanhandles.caimandr_choices.no_files
    ...
    if isfield(caimanhandles.caimandr_choices,'start_reversal')
        if caimanhandles.caimandr_choices.start_reversal==filNum
            plot([first_num_odor_trials(filNum) first_num_odor_trials(filNum)],[0 100],'-m','LineWidth',4)
            text(first_num_odor_trials(filNum)+2,80,'Reversal','Color','m','FontSize',18)
        end
    end
end
...
for winNo=1:szwins(1)
    ...
    if isfield(caimanhandles.caimandr_choices,'start_reversal')
        ...
        plot([first_num_odor_trials(filNum) first_num_odor_trials(filNum)],[prctile(mean_win_dFF(:),1)-0.1*(prctile(mean_win_dFF(:),99)-prctile(mean_win_dFF(:),1)) 1.2*prctile(mean_win_dFF(:),99)-0.1*(prctile(mean_win_dFF(:),99)+prctile(mean_win_dFF(:),1))],'-m','LineWidth',4)
        text(first_num_odor_trials(filNum)+2,prctile(mean_win_dFF(:),1)+0.9*(prctile(mean_win_dFF(:),99)-prctile(mean_win_dFF(:),1)),'Reversal','Color','m','FontSize',18)
    end
...
save([caimanhandles.caimandr_choices.outPathName caimanhandles.caimandr_choices.outFileName(1:end-4) '_bpsout.mat'],'handles_out')
