...
[fname,pname,nCancel] = uigetfile({'*.mat'},'Select the dropcspm_hf or dropcnsampler output file...');
if nCancel
...
%                                 splus_traces(spii,1:sum(snip_mask))=traces(trNo,snip_mask);
                                od_ii=od_ii+1;
...
%                                 sminus_traces(smii,1:sum(snip_mask))=traces(trNo,snip_mask);
                                od_ii=od_ii+1;
                              odor_traces(od_ii,1:sum(snip_mask))=traces(trNo,snip_mask)-mean(traces(trNo, ref_mask));
%                                 odor_traces(od_ii,1:sum(snip_mask))=traces(trNo,snip_mask);
                                dt_trace=min([dt_trace dt*sum(snip_mask)]);
...
%                             Hit_traces(Hitii,1:sum(snip_mask))=traces(trNo,snip_mask);
...
%                             Miss_traces(Missii,1:sum(snip_mask))=traces(trNo,snip_mask);
...
%                             FA_traces(FAii,1:sum(snip_mask))=traces(trNo,snip_mask);
...
%                             CR_traces(CRii,1:sum(snip_mask))=traces(trNo,snip_mask);
...
%To plot a specific trace
%which_trace_Hit is the ROI
%Hit_trials is the trial within hit
% Hit_traces((which_trace_Hit==ROIno)&(Hit_trials==Hit_trialNo),:)
% odor_snip=(time_to_eventHit>=0)&(time_to_eventHit<=5);
% for ROIno=1:no_traces
%     for Hit_trialNo=1:no_Hit_trials
%         dFF_odor(ROIno,Hit_trialNo)=mean(Hit_traces((which_trace_Hit==ROIno)&(Hit_trials==Hit_trialNo),odor_snip));
%     end
% end
% pctdFF_odor95=prctile(dFF_odor(:),95);
% pctdFF_odor5=prctile(dFF_odor(:),5);
