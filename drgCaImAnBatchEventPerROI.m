function drgCaImAnBatchEventPerROI(choiceBatchPathName,choiceFileName)

%This code does accounting to determine whether ROIs respond to both
%correct responses (Hits and CRs) and errors (Miss and FAs)
% Needs the choices used for LDA analysis such as
% drgCaImAnChoicesDiego20180910_mmPVG04_CerebellumLDA
...
%                     norm_dFF=win_dFF./ref_dFF;
                ...
                file_for_mean_snip_dFF(num_odor_trials_dFF)=filNum;
...
%Note that I only do this for percent correct >80%
no_trial_windows=total_trial_windows;
...
for ii=1:num_odor_trials_dFF
    if sum((lick_times(ii,1:no_licks(ii))>=t_odor_on)&(lick_times(ii,1:no_licks(ii))<=t_odor_off))<lick_threshold
        ...
        if (perCorr(ii)>=pct_windows(no_trial_windows,1))&(perCorr(ii)<pct_windows(no_trial_windows,2))
            ...
            which_files_per_trial_in_PCA(jj)=file_for_mean_snip_dFF(ii);
        else
            dFF_trial_mask(ii)=0;
        end
    else
...
%Now do accounting for responsiveness of each component
SNR=2.5;
handles_per_comp=[];
handles_per_comp.no_tseries_included=0;
fprintf(1, ['\n\nResponse analysis for ' choiceFileName '\n']);
fprintf(1, ['\nPercent correct>80%%\n\n']);

for filNum=1:caimanhandles.caimandr_choices.no_files
    
    %For this file number do we have any trials
    if sum(file_for_mean_snip_dFF(which_trials_in_PCA)==filNum)>0
        fprintf(1, ['\nFor file number %d there are %d trials\n'],filNum,sum(file_for_mean_snip_dFF(which_trials_in_PCA)==filNum));
        fprintf(1, ['Number of components %d\n'],noROIs(filNum));
        
        handles_per_comp.no_tseries_included=handles_per_comp.no_tseries_included+1;
        handles_per_comp.tseries(handles_per_comp.no_tseries_included).fileNo=filNum;
        
        %Hits
        these_dFF_trials=which_trials_in_PCA((events_miss_FA==1)&(filNum==which_files_per_trial_in_PCA));
        handles_per_comp.tseries(handles_per_comp.no_tseries_included).noHit_trials=length(these_dFF_trials);
        handles_per_comp.tseries(handles_per_comp.no_tseries_included).noROIs=noROIs(filNum);
        hit_responses=zeros(1,noROIs(filNum));
        
        if length(these_dFF_trials)>0
            no_resp_hit=0;
            fprintf(1, ['Number of Hit trials %d\n'],length(these_dFF_trials));
            for compNo=1:noROIs(filNum)
                this_mean=zeros(1,no_time_points);
                this_mean(1,:)=mean(mean_snip_dFF_per_comp(these_dFF_trials,compNo,1:no_time_points),1);
                ref_mean=mean(this_mean((time_to_eventLDA>=-10)&(time_to_eventLDA<=-2)));
                ref_SD=std(this_mean((time_to_eventLDA>=-10)&(time_to_eventLDA<=-2)));
                if sum(abs(this_mean((time_to_eventLDA>=0)&(time_to_eventLDA<=mean(delta_odor)))-ref_mean)>SNR*ref_SD)>0
                    %There is a response
                    no_resp_hit=no_resp_hit+1;
                    hit_responses(1,compNo)=1;
                end
            end
            handles_per_comp.tseries(handles_per_comp.no_tseries_included).noHit_responses= no_resp_hit;
            handles_per_comp.tseries(handles_per_comp.no_tseries_included).Hit_responses=hit_responses;
            fprintf(1, ['Number of components responding with Hits %d\n'],no_resp_hit);
        end
        
        %Miss
        these_dFF_trials=which_trials_in_PCA((events_miss_FA==2)&(filNum==which_files_per_trial_in_PCA));
        handles_per_comp.tseries(handles_per_comp.no_tseries_included).noMiss_trials=length(these_dFF_trials);
        miss_responses=zeros(1,noROIs(filNum));
        
        if length(these_dFF_trials)>0
            no_resp_miss=0;
            fprintf(1, ['Number of Miss trials %d\n'],length(these_dFF_trials));
            for compNo=1:noROIs(filNum)
                this_mean=zeros(1,no_time_points);
                this_mean(1,:)=mean(mean_snip_dFF_per_comp(these_dFF_trials,compNo,1:no_time_points),1);
                ref_mean=mean(this_mean((time_to_eventLDA>=-10)&(time_to_eventLDA<=-2)));
                ...
                if sum(abs(this_mean((time_to_eventLDA>=0)&(time_to_eventLDA<=mean(delta_odor)))-ref_mean)>SNR*ref_SD)>0
                    %There is a response
                    no_resp_miss=no_resp_miss+1;
                    miss_responses(1,compNo)=1;
                end
            end
            handles_per_comp.tseries(handles_per_comp.no_tseries_included).noMiss_responses= no_resp_miss;
            handles_per_comp.tseries(handles_per_comp.no_tseries_included).Miss_responses=miss_responses;
            fprintf(1, ['Number of components responding with Miss %d\n'],no_resp_miss);
        end
        
        %CRs
        these_dFF_trials=which_trials_in_PCA((events_miss_FA==3)&(filNum==which_files_per_trial_in_PCA));
        handles_per_comp.tseries(handles_per_comp.no_tseries_included).noCR_trials=length(these_dFF_trials);
        CR_responses=zeros(1,noROIs(filNum));
        
        if length(these_dFF_trials)>0
            no_resp_CR=0;
            fprintf(1, ['Number of CR trials %d\n'],length(these_dFF_trials));
            for compNo=1:noROIs(filNum)
                this_mean=zeros(1,no_time_points);
                this_mean(1,:)=mean(mean_snip_dFF_per_comp(these_dFF_trials,compNo,1:no_time_points),1);
                ref_mean=mean(this_mean((time_to_eventLDA>=-10)&(time_to_eventLDA<=-2)));
                ...
                if sum(abs(this_mean((time_to_eventLDA>=0)&(time_to_eventLDA<=mean(delta_odor)))-ref_mean)>SNR*ref_SD)>0
                    %There is a response
                    no_resp_CR=no_resp_CR+1;
                    CR_responses(1,compNo)=1;
                end
            end
            handles_per_comp.tseries(handles_per_comp.no_tseries_included).noCR_responses= no_resp_CR;
            handles_per_comp.tseries(handles_per_comp.no_tseries_included).CR_responses=CR_responses;
            fprintf(1, ['Number of components responding with CR %d\n'],no_resp_CR);
        end
        
        %FAs
        these_dFF_trials=which_trials_in_PCA((events_miss_FA==4)&(filNum==which_files_per_trial_in_PCA));
        handles_per_comp.tseries(handles_per_comp.no_tseries_included).noFA_trials=length(these_dFF_trials);
        FA_responses=zeros(1,noROIs(filNum));
        if length(these_dFF_trials)>0
            fprintf(1, ['Number of FAs trials %d\n'],length(these_dFF_trials));
            no_resp_FA=0;
            ...
            for compNo=1:noROIs(filNum)
                ...
                if sum(abs(this_mean((time_to_eventLDA>=0)&(time_to_eventLDA<=mean(delta_odor)))-ref_mean)>SNR*ref_SD)>0
                    %There is a response
                    no_resp_FA=no_resp_FA+1;
                    FA_responses(1,compNo)=1;
                end
            end
            handles_per_comp.tseries(handles_per_comp.no_tseries_included).noFA_responses= no_resp_FA;
            handles_per_comp.tseries(handles_per_comp.no_tseries_included).FA_responses=FA_responses;
            fprintf(1, ['Number of components responding with FA %d\n'],no_resp_FA);
        end
        
        fprintf(1, ['Number of components responding for Miss and Hits %d\n'],sum(logical(hit_responses)&logical(miss_responses)));
        handles_per_comp.tseries(handles_per_comp.no_tseries_included).noMissHit_responses=sum(logical(hit_responses)&logical(miss_responses));
        fprintf(1, ['Number of components responding for FA and Hits %d\n'],sum(logical(hit_responses)&logical(FA_responses)));
        handles_per_comp.tseries(handles_per_comp.no_tseries_included).noFAHit_responses=sum(logical(hit_responses)&logical(FA_responses));
        no_resp_corr=logical(hit_responses)|logical(CR_responses);
        no_resp_err=logical(miss_responses)|logical(FA_responses);
        fprintf(1, ['Number of components responding for correct trials and errors %d\n'],sum(no_resp_corr&no_resp_err));
    end
 
   
end

save([caimanhandles.caimandr_choices.outPathName caimanhandles.caimandr_choices.outFileName(1:end-4) '_account_events.mat'],'handles_per_comp')

%Plot one component

%Use this for mmPVG04 20180917_mmPVG04_Cerebellum
% compNo=2;
% filNum=6;
% ymin=-0.4;
% ymax=1.5;

%Use this for 20180515_18_mmPVG02
compNo=80
filNum=1;
ymin=-0.5;
ymax=1.5;
...
if length(these_dFF_trials)>0
    ...
    if length(these_dFF_trials)>=3
        CI=[];
        these_snips=zeros(length(these_dFF_trials),no_time_points);
        these_snips(:,:)=mean_snip_dFF_per_comp(these_dFF_trials,compNo,1:no_time_points);
        CI = bootci(1000, {@mean, these_snips})';
        CI(:,1)=this_mean'-CI(:,1);
        CI(:,2)=CI(:,2)-this_mean';
        
        if length(this_mean) <= length(time_to_eventLDA)
            [hlCR, hpCR] = boundedline(time_to_eventLDA(1:length(this_mean))',this_mean', CI, 'r');
        else
            this_mean=this_mean(1:length(time_to_eventLDA));
            CI=CI(1:length(time_to_eventLDA),:);
            [hlCR, hpCR] = boundedline(time_to_eventLDA',this_mean', CI, 'r');
        end
    else
        plot(time_to_eventLDA',this_mean, CI, 'r')
    end
    ...
    xlabel('Time (sec)')
    ylabel('dF/F')
    title(['Timecourse for dF/F for Hits, percent correct >80%'])
end
...
if length(these_dFF_trials)>0
    ...
    if length(these_dFF_trials)>=3
        ...
        [hlCR, hpCR] = boundedline(time_to_eventLDA(1:length(this_mean))',this_mean', CI, 'c');
    else
        minl=min([length(time_to_eventLDA) length(this_mean)]);
        plot(time_to_eventLDA(1:minl)',this_mean(1:minl), 'c')
    end
    ...
    xlabel('Time (sec)')
    ylabel('dF/F')
    title(['Timecourse for dF/F for Miss, percent correct >80%'])
end
...
if length(these_dFF_trials)>0
    ...
    if length(these_dFF_trials)>=3
        ...
        if length(this_mean) <= length(time_to_eventLDA)
            [hlCR, hpCR] = boundedline(time_to_eventLDA(1:length(this_mean))',this_mean', CI, 'b');
        else
            this_mean=this_mean(1:length(time_to_eventLDA));
            CI=CI(1:length(time_to_eventLDA),:);
            [hlCR, hpCR] = boundedline(time_to_eventLDA',this_mean', CI, 'b');
    ...
    xlabel('Time (sec)')
    ylabel('dF/F')
    title(['Timecourse for dF/F for CR, percent correct >80%'])
end
...
if length(these_dFF_trials)>0
    ...
    if length(these_dFF_trials)>=3
        ...
        if length(this_mean) <= length(time_to_eventLDA)
            [hlCR, hpCR] = boundedline(time_to_eventLDA(1:length(this_mean))',this_mean', CI, 'm');
        else
            this_mean=this_mean(1:length(time_to_eventLDA));
            CI=CI(1:length(time_to_eventLDA),:);
            [hlCR, hpCR] = boundedline(time_to_eventLDA',this_mean', CI, 'm');
        ...
        minl=min([length(time_to_eventLDA) length(this_mean)]);
        plot(time_to_eventLDA(1:minl)',this_mean(1:minl), 'm')
    end
    ...
    xlabel('Time (sec)')
    ylabel('dF/F')
    title(['Timecourse for dF/F for FA, percent correct >80%'])
end
