...
% close all
clearvars -except pre_perBatchPathName pre_perFileName p_thr_less_than p_thr_more_than MLalgo show_figures no_sp_sm_trials_to_use...
    first_sp_sm_trial_no this_cost figNo fileNo
warning('off')
...
if (handles_out.no_sp_trials>=min_no_trials)&(handles_out.no_sm_trials>=min_no_trials)
    ...
    if show_figures==1
        ...
        title(['Ca changes file No ' num2str(fileNo) ' threshold ' num2str(p_thr_less_than)])
        ...
        figname=[pre_perBatchPathName pre_perFileName];
        savefig([figname(1:end-4) '_fig' num2str(figNo-first_fig_no) '.fig'])
    end
...
if handles_out2.decoding_processed==1
    ...
    if sum((p<=p_thr_less_than)&(p>=p_thr_more_than))>0
        fprintf(1,'\ndrgCaImAn_pre_per_to_LDA_fsdz_new running for file %d threshold %d with %d ROIs\n\n'...
            ,fileNo,p_thr_less_than,sum((p<=p_thr_less_than)&(p>=p_thr_more_than)))
        gcp;
        handles_out2.decoding_processed=1;
        ...
        for time_point=1:no_timepoints
            ...
            for jj=1:Nall
                handles_out2.timepoint_processed(time_point,jj)=timepoint_processed(jj);
                handles_out2.correct_predict(time_point,jj)=correct_predict(jj);
                handles_out2.correct_predict_shuffled(time_point,jj)=correct_predict_shuffled(jj);
            end
        ...
        if show_figures==1
            ...
            title(['Decoding accuracy per timepoint for file No ' num2str(fileNo) ' threshold ' num2str(p_thr_less_than)])
            
            savefig([figname(1:end-4) '_fig' num2str(figNo-first_fig_no) '.fig'])
        end
        
        %Now do the decoding by training with all timepoints before and after odor
        %application
        ...
        training_decisions_pre=zeros(1,Nall*no_points_pre);
        
        ii=0;
        for time_point=1:ii_zero-1
            measurements_pre(ii+1:ii+Nall,:)=z_training_neural_recordings(:,:,time_point);
            training_decisions_pre(ii+1:ii+Nall)=training_decisions;
            ii=ii+Nall;
        end
        ...
        parfor ii=1:Nall_pre
            ...
            try
                ...
                %Predict labels for the test set. You trained Mdl using a table of data, but you can predict labels using a matrix.
                [label,score] = predict(Mdl,measurements_pre(logical(idxTest),:));
                labels(ii)=label;
                
                %label is the predicted label, and score is the predicted class
                %posterior probability
                
                if label==training_decisions_pre(ii)
                    correct_predict(ii)=1;
                else
                    correct_predict(ii)=0;
                end
                
                ii_shuffled=randperm(Nall_pre);
                
                if label==training_decisions_pre(ii_shuffled(ii))
                    correct_predict_shuffled(ii)=1;
                else
                    correct_predict_shuffled(ii)=0;
                end
        ...
        %Now try to use all trials after t=0 for training
        ...
        parfor ii=1:Nall_post
            ...
            try
                ...
                %Predict labels for the test set. You trained Mdl using a table of data, but you can predict labels using a matrix.
                [label,score] = predict(Mdl,measurements_post(logical(idxTest),:));
                labels(ii)=label;
                
                %label is the predicted label, and score is the predicted class
                %posterior probability
                
                if label==training_decisions_post(ii)
                    correct_predict(ii)=1;
                else
                    correct_predict(ii)=0;
                end
                
                ii_shuffled=randperm(Nall_post);
                
                if label==training_decisions_post(ii_shuffled(ii))
                    correct_predict_shuffled(ii)=1;
                else
                    correct_predict_shuffled(ii)=0;
                end
        ...
        for time_point=ii_zero:length(time)
            ...
            for jj=1:Nall
                handles_out2.timepoint_processed_pp(time_point,jj)=this_timepoint_processed(jj);
                handles_out2.correct_predict(time_point,jj)=this_correct_predict(jj);
                handles_out2.correct_predict_shuffled(time_point,jj)=this_correct_predict_shuffled(jj);
            end
            if show_figures==1
                fprintf(1, ['For timepoint %d accuracy= %d and shuffled accuracy= %d\n'],time_point,accuracy(time_point),sh_accuracy(time_point));
            end
        ...
        %Note that the accuracy output is only for the training trial set
        handles_out2.accuracy_pp=accuracy;
        ...
        handles_out2.bishop_accuracy_pp=sum(winning_label==training_decisions)/Nall;
        these_bishop_sh_accuracy=[];
        for ii_no=1:5
            shuffled_training_decisions=training_decisions(randperm(Nall));
            these_bishop_sh_accuracy(ii_no)=sum(winning_label==shuffled_training_decisions)/Nall;
        end
        handles_out2.bishop_sh_accuracy_pp=mean(these_bishop_sh_accuracy);
        
        handles_out2.winning_label_pp=winning_label;
        handles_out2.training_decisions_pp=training_decisions;
        ...
        if show_figures==1
            ...
            xlabel('Time (sec)')
            ylabel('Accuracy')
            title(['Accuracy for pre-post decoding for file No ' num2str(fileNo) ' threshold ' num2str(p_thr_less_than)])
            savefig([figname(1:end-4) '_fig' num2str(figNo-first_fig_no) '.fig'])
        end
