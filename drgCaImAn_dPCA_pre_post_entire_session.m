function handles_out=drgCaImAn_dPCA_pre_post_entire_session(handles_choices)
...
D = 2; %Number of pre stimuli (this is decision in dpca_demo)
...
trial_number=zeros(2,2);
for trNum=1:length(training_decisions)
    if (training_decisions(trNum)==1)&(training_decisions_pre(trNum)==1)
        trial_number(1,1)=trial_number(1,1)+1;
    end
    if (training_decisions(trNum)==1)&(training_decisions_pre(trNum)==2)
        trial_number(1,2)=trial_number(1,2)+1;
    end
    
    if (training_decisions(trNum)==2)&(training_decisions_pre(trNum)==1)
        trial_number(2,1)=trial_number(2,1)+1;
    end
    if (training_decisions(trNum)==2)&(training_decisions_pre(trNum)==2)
        trial_number(2,2)=trial_number(2,2)+1;
    end
end

E=max([trial_number(:)]);

firingRates=NaN(N,S,D,T,E); % firingRates: N x S x D x T x maxTrialNum

%Tranfer the data
%trial number is the number of trials for each combination of s and d
...
for trNum=1:length(training_decisions)
    if (training_decisions(trNum)==1)&(training_decisions_pre(trNum)==1)
        trial_number(1,1)=trial_number(1,1)+1;
        for n=1:N
            firingRates(n,1,1,:,trial_number(1,1))=measurements_per_trial(:,n,trNum);
        end
    end
    if (training_decisions(trNum)==1)&(training_decisions_pre(trNum)==2)
        trial_number(1,2)=trial_number(1,2)+1;
        for n=1:N
            firingRates(n,1,2,:,trial_number(1,2))=measurements_per_trial(:,n,trNum);
        end
    end
    ...
    if (training_decisions(trNum)==2)&(training_decisions_pre(trNum)==1)
        trial_number(2,1)=trial_number(2,1)+1;
        for n=1:N
            firingRates(n,2,1,:,trial_number(2,1))=measurements_per_trial(:,n,trNum);
        end
    end
    if (training_decisions(trNum)==2)&(training_decisions_pre(trNum)==2)
        trial_number(2,2)=trial_number(2,2)+1;
        for n=1:N
            firingRates(n,2,2,:,trial_number(2,2))=measurements_per_trial(:,n,trNum);
        end
    end
end
...
for n=1:N
    for s=1:S
        for d=1:D
            trialNum(n,s,d)=trial_number(s,d);
        end
    end
end
...
combinedParams = {{1, [1 3]}, {2, [2 3]}, {3}, {[1 2], [1 2 3]}};
margNames = {'Odor', 'Odor pre', 'Odor-independent', 'Odor/Odor pre Interaction'};
margColours = [23 100 171; 187 20 25; 150 150 150; 114 97 171]/256;
